...
import { decereamentQuantity, emptyCart, increamentQuantity, removecartItem } from "../redux/slices/cartSlice"
...
const Cart = () => {
  ...
  const handleDecrement = (product) => {
    if (product?.quantity == 1) {
      ...
      dispatch(decereamentQuantity(product.id));
  ...
  return (
    <>
      <Header />
      <div style={{ paddingTop: "100px" }} className='0x- 5 grow'>
        {
          userCart?.length > 0 ?
            <>
              <h1 className='text-5xl font-bold text-blue-600 ms-5'>Cart Summary..</h1>
              <div className='grid grid-cols-3 gap-4 mt-5 ms-5'>
                            ...
                            <td>
                              <div className='flex'>
                                ...
                                <input style={{ width: "40px" }} type="text" className='border p-1 rounded mx-2' value={product?.quantity}
                                  readOnly />
                                <button onClick={() => dispatch(increamentQuantity(product?.id))} className='font-bold'>+</button>

                              </div>
                            </td>
                            <td>$ {product?.totalPrice}</td>
                            <td><button onClick={() => dispatch(removecartItem(product?.id))} className='text-red-600'><i className='fa-solid fa-trash'></i></button></td></tr>
                        ))
                      }
              ...
              <img src="https://cdn.dribbble.com/userupload/32744211/file/original-888ecc665e715aa9433dcc0e35c0078c.gif"
                alt="" />
